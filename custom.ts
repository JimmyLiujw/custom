// blocks supporting a Keyestudio Infrared Wireless Module Kit
...
namespace IRnew {
    ...
    class InfraredLed {
        ...
        public sendNecLong(hex: string): void {
            ...
            const NEC_HIGH_SPACE = 1690 - this.waitCorrection - 50;
            const NEC_LOW_SPACE = 560 - this.waitCorrection - 50;
            // Decompose HEX string into two manageable 4 bit numbers
            let se = [];
            for (let i = 0; i < len; i++) {
                se.push(parseInt("0x" + hex.substr(2 + i, 1)))
            }
    ...
    function decode(markAndSpace: number): number {
        ...
        //basic.showNumber(irState.allbitRecived)
    ...
    function initIrState() {
        ...
        irState = {
            ...
            allbitRecived: 0,
            allSectionBits: 0,
              hiword: "", // TODO replace with uint32
            //  loword: 0,
            activeCommand: -1,
            repeatTimeout: 0,
            onIrButtonPressed: [],
    ...
    //% subcategory="IR Receiver"
    //% blockId=ir_datagram_received_string
    //% block="IR data hiword"
    //% weight=80
    export function hiwordr(): string {
        return irState.hiword;
    }
    /**
